...
int main(){
//	cout<<"쿼리 선택\n1.insert 2.select key 3.select not key 4.join"<<endl;
//	cin>>sel_query;
	sel_query = 4;
	cout<<"limit: ";
	cin>>limit_num;
...
void generate_dummy_data(const char* db_name, const char* ori, const char* trg){
	...
	case 1:
		...
		break;
	case 2:
		total_num=3*insert_num;
		fout<<total_num<<endl;
		select_query(insert_num,true);
		break;
	case 3:
		total_num=21*insert_num;
		fout<<total_num<<endl;
		select_query(insert_num,false);
		break;
	case 4:
		total_num=3*insert_num;
		fout<<total_num<<endl;
		select_join_query(insert_num,true,true);
		break;
	}
	cout<<total_num<<endl;
	fout.close();   
		...
		for(j=0;j<t_a->c_cnt ;j++){
			...
			for(k=i+1;k<t_cnt;k++){
				...
				for(l=0;l<t_b->c_cnt;l++){		
					if(t_b->key[l]==key2){
					sprintf(buf,"select a.%s, b.%s from %s as a join %s as b on a.%s = b.%s limit %d;",t_a->column_name[j],t_b->column_name[l],t_a->table_name, t_b->table_name,t_a->column_name[j],t_b->column_name[l], limit_num);
					//cout<<buf<<endl;
					fout<<buf<<endl;
					line_num++;
					}
	...
	for(j=0;j<t_cnt;j++){
		for(k=0;k<t_info[j].c_cnt;k++){
			if(t_info[j].key[k]==key){
				sprintf(buf,"select * from %s group by %s limit %d;",t_info[j].table_name, t_info[j].column_name[k], limit_num);
				//cout<<buf<<endl;
				fout<<buf<<endl;
				line_num++;
			}
