...
int main(int argc, char** argv){
	...
	sleep(1);
	for(int i=7;i<=8;i++){
		module_controller->push_request(temp[i]);
	}
	//	module_controller->push_request(temp[3]);
	sleep(1);
	module_controller->push_request(temp[9]);
	...
	for(int i=0;i<10;i++){
		...
		module_controller->t_control->thread_args[i].message=quit_str;	
	}	
